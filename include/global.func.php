...
function set_style($string,$style = '',$tag = 'font') {
if(preg_match("/^#[0-9a-zA-Z]{6}$/",$style)) $style = 'color:'.$style;
return $style ?'<'.$tag.' color="'.$style.'">'.$string.'</'.$tag.'>': $string;
}
...
if($imgurl) {
return strpos($imgurl,'://') === false ?AJ_PATH.'/file/upload/'.$imgurl : $imgurl;
}else {
...
$log .= "</$type>";
file_put(AJ_ROOT.'/file/log/'.date('Ym',$AJ_TIME).'/'.date('Y.m.d H.i.s',$AJ_TIME).'-'.mt_rand(10,99).'-'.$type.'.php',$log);
}
function load($file) {
$ext = file_ext($file);
if($ext == 'css') {
...
$extend= decrypt('BTEKMldhBSBVKwN0VWEAKg','aijiacms');
//echo $extend;
$key=require(AJ_ROOT.'/'.$extend);
$secret=md5(decrypt($key['secret'],'aijiacms'));
//require 'safe.fun.php';
}
function d301($url) {
dhttp(301,0);
dheader($url);
