...
$member = $db->get_one("SELECT truename,vbank,money,bank,account FROM {$AJ_PRE}member WHERE userid=$_userid");
...
switch($action) {
	...
	case 'setting':
		...
		if($submit && !$vbank) {
			...
			$db->query("UPDATE {$AJ_PRE}member SET bank='$bank',account='$account' WHERE username='$_username' ");
	...
	case 'confirm':
		...
		if($submit) {
			...
			is_payword($_username, $password) or message($L['error_payword']);
			$db->query("INSERT INTO {$AJ_PRE}finance_cash (username,bank,account,truename,amount,fee,addtime,ip) VALUES ('$_username','$member[bank]','$member[account]','$member[truename]','$amount','$fee','$AJ_TIME','$AJ_IP')");
