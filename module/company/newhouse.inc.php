...
if($itemid) {
	// print_r($itemid);exit;
	dheader($CFG['url'].'house/show.php/itemid-'.$itemid.'/');
	// header("Location: $CFG['url'].'house/show.php/itemid-'.$itemid");
	// exit;
	...
	if($items) {
		...
		// print_r("SELECT ".$MOD['fields']." FROM {$table} WHERE $condition ORDER BY edittime DESC LIMIT $offset,$pagesize");
		// exit;
