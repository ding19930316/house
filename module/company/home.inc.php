...
switch($action) {
	case 'search':
		if(check_name($homepage) && in_array($file, array('sale', 'buy', 'news', 'credit', 'rent', 'price', 'photo', 'info', 'newhouse', 'video')) && $kw) {
	...
	case 'next':
		...
		$r = $db->get_one("SELECT itemid FROM {$AJ_PRE}sell_5 WHERE username='$username' AND itemid>$itemid AND status=3");
