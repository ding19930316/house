...
$pagesize =6;
if($at=='huxing') $catids=24;
$cat=get_cat($catids);
if($catids) $condition .= $cat['child'] ? " AND catids IN (".$cat['arrchildid'].")" : " AND a.catid=$catids";	
...
$tjw=$db->query("SELECT  a.itemid, houseid,linkurl,addtime,title,b.item,b.itemid,b.thumb,b.introduce ,b.mianji FROM $a a  LEFT JOIN $b b  ON b.item = a.itemid   where a.houseid=$itemid  {$condition} ORDER BY b.itemid ASC LIMIT $offset,$pagesize");
