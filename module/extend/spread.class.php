...
class spread {
	...
	function get_list($condition = '1', $order = 'itemid DESC') {
		global $MOD, $TYPE, $pages, $page, $pagesize, $offset, $AJ_TIME, $L;
	...
	function _add($post) {
		...
		$post['newhouse_price'] = dround($post['newhouse_price']);
		$post['rent_price'] = dround($post['rent_price']);
		$post['company_price'] = dround($post['company_price']);
		$this->db->query("INSERT INTO {$this->table_price} (word,sell_price,buy_price,newhouse_price,rent_price,company_price,editor,edittime) VALUES('$post[word]','$post[sell_price]','$post[buy_price]','$post[newhouse_price]','$post[rent_price]','$post[company_price]','$_username','$AJ_TIME')");
	}
	...
	function _edit($post) {
		foreach($post as $k=>$v) {
			...
			$v['newhouse_price'] = dround($v['newhouse_price']);
			$v['rent_price'] = dround($v['rent_price']);
			$v['company_price'] = dround($v['company_price']);
			$this->db->query("UPDATE {$this->table_price} SET word='$v[word]',sell_price='$v[sell_price]',buy_price='$v[buy_price]',newhouse_price='$v[newhouse_price]',rent_price='$v[rent_price]',company_price='$v[company_price]' WHERE itemid='$k'");
		}
	}
