<?php defined('IN_AIJIACMS') or exit('Access Denied');?><div class="houseinfo mt10">
      <h3><span>最新<?php echo $main_name[$HM];?></span><a href="<?php echo userurl($username, 'file=rent', $domain);?>">更多>></a></h3>
      <ul class="house">
  <?php $tags=tag("moduleid=7&condition=status>2 and username='$username' &pagesize=".$main_num[$HM]."&order=edittime desc&fields=*&length=56&template=null");?>
...
，楼层：<?php echo $t['floor1'];?>/<?php echo $t['floor2'];?></li>
...
<?php if($t['skype'] && $AJ['im_skype']) { ?><?php echo im_skype($t['skype']);?>&nbsp;<?php } ?>
              ...
              <div class="b_elist_lloupantxt2 fl"><?php if($t['houseearm']) { ?><?php echo $t['houseearm'];?>平米<?php } else { ?>暂无<?php } ?>
                ...
                <div class="b_elist_lloupantxt3 fl"><?php if($t['price']>0) { ?><?php echo $t['price'];?>元/月<?php } else { ?>面议<?php } ?>
</div>
                <div class="clear"></div>
            </div>
