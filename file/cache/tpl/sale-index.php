...
<div id="main">
<div class="bread">您当前的位置2：<a href="<?php echo $MODULE['1']['linkurl'];?>" title="<?php echo $AJ['sitename'];?>"><?php echo $AJ['sitename'];?></a>&gt;<a href="<?php echo $MOD['linkurl'];?>"><?php echo $MOD['name'];?></a>&gt;<span>房源搜索</span></div>
...
<input type="number" value="<?php echo $maxprice;?>"> 万
...
<li><a <?php if(empty($fix)) { ?>class="c"<?php } ?>
 href="<?php echo $MOD['linkurl'];?>list<?php echo deal_str($lst,'v');?>.html">不限</a></li>
                  <?php $fix_arr = getbox_name('fix','sale_5')?>
...
<li><a href="<?php echo $MOD['linkurl'];?>list<?php echo deal_str($lst,'s');?>.html">不限</a></li>
                                                     <?php $toward_arr = getbox_name('toward','sale_5')?>
...
<h2>
<span class="hl_page">
共为您找到 <b class="red"><?php $esfcount=$db->get_one("select COUNT(*) AS num from ".$AJ_PRE."sale_5 wher&#101; status=3");?>
<?php echo $esfcount['num'];?></b> 个房源<a class="pre" href="<?php echo $MOD['linkurl'];?>list<?php echo deal_str($lst,'g');?>-g<?php if($page==1) { ?>1<?php } else { ?><?php echo $page-1;?><?php } ?>
...
<?php if($ord==5) { ?>up<?php } ?>
">单价</a>
<a href="<?php if($ord==6) { ?>list<?php echo deal_str($lst,'n');?>-n7.html<?php } else { ?>list<?php echo deal_str($lst,'n');?>-n6.html<?php } ?>
"  class="A1 <?php if($ord==6) { ?>down<?php } ?>
<?php if($ord==7) { ?>up<?php } ?>
">总价</a>
</div>
</div>
                                  <?php echo tag("moduleid=$moduleid&condition=status=3$dtype&areaid=$cityid&catid=$catid&pagesize=".$MOD['pagesize']."&page=$page&showpage=1&datetype=5&order=".$MOD['order']."&fields=".$MOD['fields']."&template=list-sale");?>
</div>
<div class="hlist_fr">
...
<?php if(is_array($browseHouse)) { foreach($browseHouse as $k => $t) { ?> <tr>
                                        <td><a target="_blank" href="<?php echo $t['linkurl'];?>" title="<?php echo $t['title'];?>"><?php echo dsubstr($t['housename'], 16, $suffix = '..');?> </a></td>
