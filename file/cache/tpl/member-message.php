...
<link rel="stylesheet" type="text/css" href="<?php echo AJ_STATIC;?><?php echo $MODULE['2']['moduledir'];?>/image/message.css"/>
<div class="tinfo">
...
<input type="submit" name="submit" value=" 确 定 " class="sBtn"/>&nbsp;&nbsp;&nbsp;&nbsp;<input type="reset" name="reset" value=" 重 置 " class="sBtn"/>
...
<td class="tl"> </td>
<td class="tr" height="50">
<input type="submit" name="submit" value=" 导 出 " class="sBtn"/>&nbsp;&nbsp;&nbsp;&nbsp;<input type="reset" name="reset" value=" 重 置 " class="sBtn"/>
</td>
</tr>
</table>
</form>
...
<td class="tl"> </td>
<td class="tr" height="50">
<input type="submit" name="submit" value=" 清 理 " class="sBtn" onclick="if(!confirm('确定要清理吗？此操作将不可撤销')) return false;"/>&nbsp;&nbsp;&nbsp;&nbsp;<input type="reset" name="reset" value=" 重 置 " class="sBtn"/>
</td>
</tr>
</table>
</form>
...
<?php echo $content;?><?php if($typeid==3) { ?><br>预约看房联系人：<?php echo $truename;?>&nbsp;&nbsp;&nbsp; 联系电话：<?php echo $telephone;?><?php } ?>
</div>
<div class="message_foot">
<input type="button" value=" 返 回 " class="sBtn" onclick="history.back(-1);"/>
<?php if($status==4) { ?>
<input type="button" class="sBtn" value="还 原" onclick="Go('?action=restore&itemid=<?php echo $itemid;?>&forward=<?php echo urlencode('?action=inbox');?>');"/> 
<?php if($fromuser) { ?>
<input type="button" class="sBtn" value="回 复" onclick="Go('?action=send&touser=<?php echo $fromuser;?>&title=RE:<?php echo urlencode($title);?>');"/> 
<input type="button" class="sBtn" value="转 发" onclick="Dd('_send').submit();"/>
<input type="button" class="sBtn" value="拒 收" onclick="if(confirm('确定要拒绝来自 <?php echo $fromuser;?> 的站内信吗？'))Go('?action=refuse&username=<?php echo $fromuser;?>');"/>  
<?php } ?>
<input type="button" class="sBtn" value="彻底删除" onclick="if(confirm('确定要删除此信件吗？此操作不可撤销')) Go('?action=delete&itemid=<?php echo $itemid;?>&forward=<?php echo urlencode('?action=recycle');?>');"/> 
<?php $action='recycle'?>
...
<input type="button" class="sBtn" value="拒 收" onclick="if(confirm('确定要拒绝来自 <?php echo $fromuser;?> 的站内信吗？'))Go('?action=refuse&username=<?php echo $fromuser;?>');"/>  
<?php } ?>
<input type="button" class="sBtn" value="回收站" onclick="Go('?action=delete&itemid=<?php echo $itemid;?>&forward=<?php echo urlencode('?action=inbox');?>');"/>
<input type="button" class="sBtn" value="彻底删除" onclick="if(confirm('确定要删除此信件吗？此操作不可撤销')) Go('?action=delete&recycle=0&itemid=<?php echo $itemid;?>&forward=<?php echo urlencode('?action=inbox');?>');"/> 
<?php $action='inbox'?>
<?php } else if($status==2) { ?>
 <?php if($fromuser) { ?>
 <input type="button" class="sBtn" value="转 发" onclick="Dd('_send').submit();"/>
 <?php } ?>
 <input type="button" class="sBtn" value="彻底删除" onclick="if(confirm('确定要删除此信件吗？此操作不可撤销')) Go('?action=delete&itemid=<?php echo $itemid;?>&forward=<?php echo urlencode('?action=outbox');?>');"/> 
<?php $action='outbox'?>
<?php } else if($status==1) { ?>
 ...
 <input type="button" class="sBtn" value="彻底删除" onclick="if(confirm('确定要删除此信件吗？此操作不可撤销')) Go('?action=delete&itemid=<?php echo $itemid;?>&forward=<?php echo urlencode('?action=draft');?>');"/>
<?php $action='draft'?>
...
<th width="532">&nbsp;标 题</th>
<th width="150">发件人</th>
<th width="270" align="center">时 间</th>
</tr>
...
<td><a href="?action=<?php echo $action;?>&typeid=<?php echo $message['typeid'];?>">
<img src="<?php echo AJ_STATIC;?><?php echo $MODULE['2']['moduledir'];?>/image/message_<?php echo $message['typeid'];?>.gif" width="16" height="16" title="<?php echo $NAME[$message['typeid']];?>" alt=""/></a></td>
...
<input type="submit" name="submit" value=" 更 新 " class="sBtn"/>&nbsp;&nbsp;&nbsp;&nbsp;<input type="button" value=" 清 空 " class="sBtn" onclick="Dd('black').value='';"/>&nbsp;&nbsp;&nbsp;&nbsp;<input type="reset" name="reset" value=" 重 置 " class="sBtn"/>
</td>
</tr>
</table>
</form>
...
<tr>
<th width="48"><input type="checkbox" class="hid" onclick="checkall(this.form);"/></th>
<th width="80">类型</th>
<th  width="300">&nbsp;标 题</th>
...
<?php if($status==3) { ?>
<th width="110" align="center">标记</th>
<?php } ?>
</tr>
<?php if($status==3) { ?>
...
<td><img src="<?php echo AJ_STATIC;?><?php echo $MODULE['2']['moduledir'];?>/image/message_sys.gif" width="16" height="16" title="系统广播" alt=""/></td>
...
<td><a href="?action=<?php echo $action;?>&typeid=<?php echo $message['typeid'];?>"><img src="<?php echo AJ_STATIC;?><?php echo $MODULE['2']['moduledir'];?>/image/message_<?php echo $message['typeid'];?>.gif" width="16" height="16" title="<?php echo $NAME[$message['typeid']];?>" alt=""/></a></td>
<td align="left"><a href="<?php if($status==1) { ?>?action=edit&itemid=<?php echo $message['itemid'];?><?php } else { ?>?action=show&itemid=<?php echo $message['itemid'];?><?php } ?>
...
<div class="sBtns">
<?php if($status==4) { ?>
<input type="submit" value=" 还 原 " class="sBtn" onclick="this.form.action='?action=restore';"/>
<input type="submit" value=" 彻底删除 " class="sBtn" onclick="if(confirm('确定要删除选中信件吗？此操作不可撤销')){this.form.action='?action=delete';}else{return false;}"/>
<?php } else if($status==3) { ?>
<span class="f_r">
<input type="text" value="输入编号,如135" size="15" id="m_id" onfocus="if(this.value=='输入编号,如135')this.value='';"/>
<input type="button" value=" 查 看 " class="sBtn" onclick="if(Dd('m_id').value.match(/^[0-9]{1,}$/)){Go('?action=show&itemid='+Dd('m_id').value);return;}Dd('m_id').value='';Dd('m_id').focus();"/>
</span>
<input type="submit" value=" 标记已读 " class="sBtn" onclick="this.form.action='?action=mark';"/>
<input type="submit" value=" 回收站 " class="sBtn" onclick="this.form.action='?action=delete';"/>
<input type="submit" value=" 彻底删除 " class="sBtn" onclick="if(confirm('确定要删除选中信件吗？此操作不可撤销')){this.form.action='?action=delete&recycle=0';}else{return false;}"/>
<?php } else { ?>
<input type="submit" value=" 彻底删除 " class="sBtn" onclick="if(confirm('确定要删除选中信件吗？此操作不可撤销')){this.form.action='?action=delete';}else{return false;}"/>
<?php } ?>
<input type="submit" value=" 清 空 " class="sBtn" onclick="if(confirm('确定要清空<?php echo $name;?>信件吗？此操作不可撤销')){this.form.action='?action=clear&status=<?php echo $status;?>';}else{return false;}"/>
</div>
</form>
</div>
<?php if($status==3) { ?>
